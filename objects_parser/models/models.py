...
class Annotation(ObjectModel):
    ...
    def __str__(self):
        if isinstance(self.classname, list):
            camel_case_types = ", ".join(
                self.type_string_to_default_type(classname)
                for classname in self.classname
            )
            camel_case_types = f"typing.Union[{camel_case_types}]"
        else:
            camel_case_types = snake_case_to_camel_case(self.classname)
        if camel_case_types == "Array":
        ...
        elif not isinstance(self.classname, list):
        ...
        if self.required:
            ...
            return f": typing.Optional[{self.classname}] = None"
